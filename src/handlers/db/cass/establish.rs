...
use cdrs_tokio::cluster::session::new_lz4 as SUPAPPClusterSession;
...
//-- cause we don't know what type of object Error trait is implemented for, for example it might be for SUPAPPClusterSession object.
...
pub async fn connection() -> Result<Arc<CassSession>, Box<dyn std::error::Error>>{
    ...
    let session = SUPAPPClusterSession(&cluster_config, load_balancer, policy).await.expect("⚠️ can't create session"); //-- SUPAPPClusterSession returns an error Result so we can call expect() on it 
    let create_ks: &'static str = "CREATE KEYSPACE IF NOT EXISTS supapp WITH REPLICATION = {'class' : 'SimpleStrategy', 'replication_factor' : 3};";
    session.query(create_ks).await.expect("⚠️ supapp keyspace create error");
